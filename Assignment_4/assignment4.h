...
void deallocate(struct mem_info* target, char* mem_area, char* name, size_t* qty){
    size_t indicate = 0;
    for(size_t i = 0; i < *qty; i++){
        if(!indicate && !strcmp(target[i].name, name)){
            ...
            if(i != *qty){
                for(size_t j = 0; j < target[i - 1].e_pos - target[i - 1].s_pos; i++)
                ...
                for(size_t j = target[i].s_pos; j < target[i].e_pos; i++)
                    *(mem_area + j) = '\0';
                *qty--;
            }
        }
