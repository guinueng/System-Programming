...
char* bin_to_uchar(char* bin_line, char* return_line, size_t target_size){
    size_t line_len = strlen(bin_line);
    size_t bin_size = 8 * target_size;
    size_t tot_qty = line_len / bin_size;
    size_t start_pos = strlen(return_line);

    unsigned char* return_mem_add = realloc(return_line, (start_pos + (tot_qty * 4) + 1) ); // During realloc, mem add can change due to limited size of original mem location.
    if(return_mem_add == NULL)
        printf("Mem reallocation failed.\n");
    return_line = return_mem_add; // Updating mem add because it can vary during realloc.
    ...
    for(size_t trial = 1; trial <= tot_qty; trial++){
        ...
        char *uchar_to_string;
        uchar_to_string = malloc(4); // Unsigned char's max value = 255. Thus, maximum 4 location is needed.
        sprintf(uchar_to_string, "%u", result); // Converting unsigned char's integer value into string.

        memcpy(return_line + strlen(return_line), uchar_to_string, sizeof(uchar_to_string)); // Copying converted string into return_line string.
        char blank[2] = {' ', '\0'}; // Making array of blank space and null to input on last part of string.
        memcpy(return_line + strlen(return_line), &blank, 2); // Copying blank and null into last part of string.
    }
    char null_ = '\0';
